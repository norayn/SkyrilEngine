...
function LogField( Space, Field, Value, StopReqursive, LogFunc )
	for i = 1, #StopReqursive do
		if Value == StopReqursive[ i ] then
			LogFunc( Space .. Field .. " = { reqursive table }," )
			return
		end
	end
	
	if type( Value ) == "string" then
		LogFunc( Space .. Field .. " = \"" .. Value .. "\"," )
	elseif type( Value ) == "function" then		
		LogFunc( Space .. Field .. " = \"" .. "function" .. "\"," )
	elseif type( Value ) == "userdata" then		
		LogFunc( Space .. Field .. " = " .. "==User data==" .. "," )
	elseif type( Value ) == "boolean" then
		local BoolVal = "false"
		if Value then BoolVal = "true" end
		LogFunc( Space .. Field .. " = " .. BoolVal .. "," )
	elseif type( Value ) ~= "table" then
		LogFunc( Space .. Field .. " = " .. Value .. "," )
	else
		table.insert( StopReqursive, Value )
		LogFunc( Space .. Field .. " = {" )
		for i, v in pairs( Value ) do LogField( "	"..Space, i, v, StopReqursive, LogFunc ) end
		LogFunc( Space .. "}," )
	end
end


function LogVar( Var, LogFunc )
	if type( Var ) == "nil" then
		LogFunc( "nil" )
	elseif type( Var ) ~= "table" then
		LogFunc( tostring( Var ) )
	else
		local StopReqursive = { Var }
		LogFunc( "{" )
		for i, v in pairs( Var ) do LogField( "	", i, v, StopReqursive, LogFunc ) end
		LogFunc( "}" )
	end
end
...
function TextFileSave(tbl,filename)
   local f,err = io.open(string.gsub(filename, '/', '\\'),"w")
   if not f then
      return nil,err
   end
