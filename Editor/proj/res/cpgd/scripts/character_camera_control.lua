Script = function( Param )
	...
	if Param.Scene.OwnerObj then 
		ObjPosition = Param.Scene.OwnerObj.Offset			
	end

	Camera.Offset = Sub( ObjPosition, Div( Camera.Size, { 2, 2 } ) )
	Camera.Offset[ 2 ] = Camera.Offset[ 2 ] - 100

	Camera.Offset[ 1 ] = math.ceil( Camera.Offset[ 1 ] * -Camera.Scale )
	Camera.Offset[ 2 ] = math.ceil( Camera.Offset[ 2 ] * -Camera.Scale )
